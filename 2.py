...
# 设置API密钥和秘密
with open('doge_id.json', 'r') as file:
    data = json.load(file)
...
def message_handler(_, message):
    ...
    try:
        ...
        if last_btc_price is not None and not stop_threads:
            ...
            if price_change_percent > 0.1 and usdt_balance > 3 and time_diff > 5:
                if 0.2 > price_change_percent > 0.12:
                    if last_btc_price < minu_low and current_btc_price < last_sell_price and time_diff > 180:
                        buy_doge()
                        print(
                    ...
                    if (last_btc_price < minu_low or current_btc_price < last_sell_price) and time_diff > 30:
                        buy_doge()
                        print(
                            f' buy_doge{last_btc_price},  {current_btc_price},  percent={price_change_percent:.2f}')
                        send_wechat_message(f"BTC Price increased {price_change_percent:.1f}%,  Buying {type_b},  "
